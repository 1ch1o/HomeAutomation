 ...
 */
 void setupOTA(){
  DEBUG_PRINTLN(F("INFO: Setting up OTA"));  
  ArduinoOTA.setHostname(OTA_HOSTNAME);
  DEBUG_PRINT(F("OTA_HOSTNAME: "));
  DEBUG_PRINTLN(OTA_HOSTNAME);
  ArduinoOTA.setPort(OTA_PORT);
  DEBUG_PRINT(F("OTA_PORT: "));
  ...
  ArduinoOTA.onStart([]() {
    ...
    DEBUG_PRINT(F("INFO: Start updating "));
    DEBUG_PRINTLN(type);
  });
  ArduinoOTA.onEnd([]() {
    DEBUG_PRINTLN(F("INFO: OTA update successfulf"));
  });
  ...
  ArduinoOTA.onError([](ota_error_t error) {
    ...
    if (error == OTA_AUTH_ERROR) {
      DEBUG_PRINTLN(F("ERROR: Auth Failed"));
    } else if (error == OTA_BEGIN_ERROR) {
      DEBUG_PRINTLN(F("ERROR: Begin Failed"));
    } else if (error == OTA_CONNECT_ERROR) {
      DEBUG_PRINTLN(F("ERROR: Connect Failed"));
    } else if (error == OTA_RECEIVE_ERROR) {
      DEBUG_PRINTLN(F("ERROR: Receive Failed"));
    } else if (error == OTA_END_ERROR) {
      DEBUG_PRINTLN(F("ERROR: End Failed"));
    }
  });
  ArduinoOTA.begin();
}  
...
char MQTT_SENSOR_JSON_TOPIC[sizeof(MQTT_SENSOR_JSON_TOPIC_TEMPLATE)-1 + sizeof(MQTT_LOCATION)-1] = {0};
char MQTT_DEBUG_TOPIC[sizeof(MQTT_DEBUG_TOPIC_TEMPLATE)-1 + sizeof(MQTT_LOCATION)-1] = {0};
/*
...
void setupMQTT(){
  ...
  DEBUG_PRINT(F("INFO: MQTT_LOCATION: "));
  ...
  ////Json-Topic
  sprintf(MQTT_SENSOR_JSON_TOPIC, MQTT_SENSOR_JSON_TOPIC_TEMPLATE, MQTT_LOCATION);

  DEBUG_PRINT(F("INFO: MQTT json topic: "));
  DEBUG_PRINTLN(MQTT_SENSOR_JSON_TOPIC); 

  ////Debug-Topic
  sprintf(MQTT_DEBUG_TOPIC, MQTT_DEBUG_TOPIC_TEMPLATE, MQTT_LOCATION);

  DEBUG_PRINT(F("INFO: MQTT debug topic: "));
  DEBUG_PRINTLN(MQTT_DEBUG_TOPIC); 
  
//// init the MQTT connection
  DEBUG_PRINT(F("INFO: Set MQTT-Server: "));
...
void checkTopicsAndPayloadsForActions(char*p_topic, String payload){
  DEBUG_PRINTLN(F("INFO: checking for actions to do"));
  ...
  //If p_topic = Command_Topic then switch relay to on and publish new state
    if (payload == MQTT_PAYLOAD_ON) {
      DEBUG_PRINT(F("INFO: following message wrote to serial: "));
    ...
    //If p_topic = Command_Topic then switch relay to off and publish new state
      ...
      DEBUG_PRINTLN(F("INFO: Nothing to do"));
    }
  }
  //If nothing to do
  else {
  ...
  if (!mqttClient.connected()) {
    if (lastMQTTConnection < millis()) {
      if (mqttClient.connect(MQTT_CLIENT_ID, MQTT_USERNAME, MQTT_PASSWORD, MQTT_AVAILABILITY_TOPIC, 0, 1, MQTT_PAYLOAD_UNAVAILABLE)) {
        DEBUG_PRINTLN(F("INFO: The client successfully connected to the MQTT broker"));
...
void subscribeTopic(char* p_topic) {
  ...
  DEBUG_PRINT(F("INFO: Subscribed to topic: "));
  DEBUG_PRINTLN(p_topic);
  } else {
    DEBUG_PRINT(F("ERROR: Could not subscribe to topic: "));
    DEBUG_PRINTLN(p_topic);
    DEBUG_PRINT(F("INFO: Either connection lost, or message too large"));
  }
}
///////////////////////////////////////////////////////////////////////////
//   SETUP() & LOOP()
